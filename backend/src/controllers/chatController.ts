...
import { AuthenticatedRequest, ApiResponse, OpenAIMessage } from '../types';
...
export class ChatController {
  ...
  static async sendMessage(req: AuthenticatedRequest, res: Response<ApiResponse>): Promise<void> {
    try {
      ...
      const recentMemories = await MemoryEntry.find({ userId })
...
Provide accurate, helpful responses while maintaining the futuristic, neural interface persona.`;
  ...
  static async getConversations(req: AuthenticatedRequest, res: Response<ApiResponse>): Promise<void> {
    try {
      ...
      const conversations = await ChatConversation.getUserConversations(userId, page, limit);
   ...
   */
  static async updateConversation(req: AuthenticatedRequest, res: Response<ApiResponse>): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({
          success: false,
   ...
   * Delete conversation
   */
  static async deleteConversation(req: AuthenticatedRequest, res: Response<ApiResponse>): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({
          success: false,
   ...
   */
  private static async extractAndStoreMemory(
    userId: string,
    userMessage: string,
    aiResponse: string
  ): Promise<void> {
    try {
      ...
      let memoryType: 'preference' | 'context' | 'skill' | 'project' | 'note' = 'note';
      ...
      const memoryData = await OpenAIService.extractMemoryKeyValue(
        `User: ${userMessage}\nAI: ${aiResponse}`,
        memoryType
      );
      ...
      if (memoryData) {
        ...
        logger.info('Memory extracted and stored', {
          userId,
          memoryType,
          key: memoryData.key
        });
      ...
      logger.error('Failed to extract memory', { error });
